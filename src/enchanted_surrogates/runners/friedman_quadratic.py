...
class FriedmanQuadratic(Runner):
    """
    FriedmanQuadratic: An implementation of the Friedman #1 regression benchmark 
    ...
    def single_code_run(self, run_dir: str, params: dict = None) -> dict:
        ...
        params = params or {}
        outfile = os.path.join(run_dir, "friedman_quadratic.csv")

        # 1. Strict Parameter Extraction
        required_keys = ['friedman_interaction_noisy'] + ["x3"]
        missing_keys = [k for k in required_keys if k not in params]
        
        if missing_keys:
            raise ValueError(
                f"FriedmanQuadratic missing required parameters: {missing_keys}. "
        ...
        try:
            ...
            raise TypeError(
                f"FriedmanQuadratic requires numeric inputs for x3. "
                f"Check parameter types in: {params}. Error: {exc}"
            )
        
        # 2. Deterministic Calculation (y_true)
        # term_interaction = 10 * np.sin(np.pi * x[0] * x[1])
        term_quadratic   = 20 * (x[0] - 0.5)**2
        # term_linear      = (10 * x[3]) + (5 * x[4])
        
        y_true = params['friedman_interaction_noisy'] + term_quadratic #+ term_linear
        ...
        df = pd.DataFrame({'friedman_interaction_plus_quadratic_noisy': [y_observed], 'friedman_interaction_plus_quadratic_true':[y_true], 'friedman_quadratic_true':[term_quadratic]})
        df.to_csv(outfile)
        ...
        # All the useful output will in in the outputed files
        # at least the run_dir should be passed so child runners know where the files are
        result = {
            "success": True,
            "friedman_quadratic_run_dir": run_dir,
        }
