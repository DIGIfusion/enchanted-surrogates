...
class FriedmanInteraction(Runner):
    """
    FriedmanInteraction: An implementation of the Friedman #1 regression benchmark 
    ...
    def single_code_run(self, run_dir: str, params: dict = None) -> dict:
        ...
        params = params or {}
        outfile = os.path.join(run_dir, "friedman_interaction.txt")

        # 1. Strict Parameter Extraction
        required_keys = [f"x{i}" for i in range(1, 3)]
        ...
        try:
            ...
            raise TypeError(
                f"FriedmanInteraction requires numeric inputs for x1-x2. "
        ...
        result = {
            "friedman_interaction_noisy": y_observed, 
            "friedman_interaction_true": y_true,  # Useful for calculating Regret or true MSE in tests
            "success": True
        }
