...
class DaskNestedExecutor(Executor):
    ...
    def __init__(
        self, base_run_dir, executors: dict, sampler_config: dict, *args, **kwargs
    ):
        """
        TODO: add docstring
        """
        ...
        self.executor_types = [
            exe_config["type"] for exe_config in self.executors_config
        ]
        self.runner_types = [
            executor_config["runner_config"]["type"]
            for executor_config in self.executors_config
        ]
        self.dask_worker_std_out_dirs = [
            os.path.join(self.base_run_dir, f"worker_out_{self.runner_types[i]}_{i}")
            for i in range(len(executors))
        ]
        log.info(
            "THE EXECUTORS WILL BE RAN WITH THESE CODES IN THE FOLLOWING ORDER:\n",
            self.runner_types,
        )
        self.executors = []
        self.reuse_bool = []
        ...
        for exe_type, exe_config in zip(self.executor_types, self.executors_config):
            ...
            if exe_type in self.executor_names:
                ...
                if index >= len(self.executors):
                    raise RuntimeError(
                        "YOU ARE TRYING TO REUSE AN EXECUTOR THAT HAS NOT YET BEEN CREATED, YOU CAN ONLY REUSE ALREADY MADE EXECUTORS"
                    )
        ...
        self.sampler = import_sampler(
            sampler_type=sampler_type, sampler_config=self.sampler_config
        )  # getattr(importlib.import_module(f'enchanted_surrogates.samplers'),sampler_type)(**sampler_config)
        ...
        self.shutdown_finished_clusters = kwargs.get(
            "shutdown_finished_clusters", False
        )
    ...
    def start_runs(self):
        ...
        if os.path.exists(os.path.join(self.base_run_dir, "ENCHANTED.FINISHED")):
            raise FileExistsError(
                f"""The file: {self.base_run_dir}/ENCHANTED.FINISHED, exists.
        ...
        for i, executor in enumerate(self.executors):
            ...
            if i == 0:
                ...
                unique_df = self.current_samples_df.drop_duplicates(
                    subset=sampler_i_params
                )
                filtered_df = unique_df[sampler_i_params]
                sampler_i_samples = filtered_df.to_dict(orient="records")
                futures, fut_to_rundir = self.executors[i].submit_batch(
                    sampler_i_samples,
                    base_run_dir=batch_dir,
                    include_fut_to_rundir=True,
                )
                ...
                sampler_cumulative_params = list(
                    set(
                        sampler_cumulative_params
                        + previous_sampler_params
                        + sampler_i_params
                    )
                )
                if self.start_cluster_when_needed:
                    ...
                    while not previous_success:
                        done_status = [
                            future.done() for future in self.all_futures[i - 1]
                        ]
                        if any(done_status):
                            prelim_results = [
                                self.get_result(future, timeout=2, silent=False)
                                for future in self.all_futures[i - 1]
                            ]
                            suc = []
                            ...
                            if any(suc):
                                ...
                                log.info(
                                    f"{datetime.now()} ONE FUTURE HAS COMPLETED WITH SUCCESS FOR NESTED DEPTH:",
                                    i - 1,
                                )
                        time.sleep(1)

                log.info(
                    f"{datetime.now()} STARTING CLUSTER: {i} FOR {executor.runner_config['type']}"
                )
                ...
                if self.reuse_bool[i]:
                    ...
                    executor.start_cluster(
                        slurm_out_dir=self.dask_worker_std_out_dirs[i]
                    )  # If block_until_cluster_started = True, this can cause issues with the scheduler not being responsive enough for dask
                ...
                while futures_check:
                    for j, future in enumerate(done):
                        ...
                        previous_sample = {
                            k: result[k] for k in previous_sampler_params if k in result
                        }
                        ...
                        enchanted_dataset_path = os.path.join(
                            os.path.dirname(run_dir), f"enchanted_dataset_{i-1}.csv"
                        )
                        dfi = pd.DataFrame({r: [v] for r, v in result.items()})
                        if os.path.exists(enchanted_dataset_path):
                            dfi.to_csv(
                                enchanted_dataset_path,
                                mode="a",
                                header=False,
                                index=False,
                            )
                        ...
                        sub_futures, fut_to_rundir = self.executors[i].submit_batch(
                            sampler_i_samples,
                ...
                if self.shutdown_finished_clusters:
                    ...
                    log.info(
                        f"STATUS OF CLUSTER: {i-1} | {self.executors[i-1].runner_config['type']} | {cluster_status}"
                    )
        ...
        base_enchanted_dataset_path = os.path.join(
            self.base_run_dir, f"enchanted_dataset.csv"
        )
        ...
        while futures_check:
            for j, future in enumerate(done):
                ...
                enchanted_dataset_path = os.path.join(
                    ...
                    f"enchanted_dataset_{len(self.executors)-1}.csv",
                )
                dfi = pd.DataFrame({r: [v] for r, v in result.items()})
                if os.path.exists(enchanted_dataset_path):
                    dfi.to_csv(
                        enchanted_dataset_path, mode="a", header=False, index=False
                    )
                else:
                    dfi.to_csv(
                        enchanted_dataset_path, mode="w", header=True, index=False
                    )

                if os.path.exists(base_enchanted_dataset_path):
                    dfi.to_csv(
                        base_enchanted_dataset_path, mode="a", header=False, index=False
                    )
                else:
                    dfi.to_csv(
                        base_enchanted_dataset_path, mode="w", header=True, index=False
                    )
        ...
        if self.shutdown_finished_clusters:
            ...
            log.info(
                f"STATUS OF CLUSTER: {len(self.executors)-1} | {executor.runner_config['type']} | {cluster_status}"
            )
        ...
        log.info(
            "WRITTING ENCHANTED.FINISHED FILE, SEE base_run_dir:", self.base_run_dir
        )
            ...
            while not result and time.time() - started < timeout:
                ...
                if run_dir:
                    ...
                    if os.path.exists(result_dir):
                        ...
                        if not silent:
                            log.warning(
                                "ENCHANTED DATA POINT FILE DOES NOT EXIST, FUTURE NOT FINISHED:",
                                result_dir,
                            )
                else:
                    raise RuntimeError(
                        f"THIS FUTURE HAS BEEN SUBMITTED BUT THE RUN_DIR WAS NOT ADDED TO fut_to_rundir, future.key: {future.key}"
                    )
                if not result:
                    if timeout == 0:
                        break
                    if not silent:
                        log.info(
                            "SLEEPING TO SEE IF THE RESULT WILL BECOME AVAILABLE, sec passed:",
                            time.time() - started,
                        )
                    time.sleep(1)
        return result
