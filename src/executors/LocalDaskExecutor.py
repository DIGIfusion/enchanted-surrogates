...
class LocalDaskExecutor(Executor):
    """
    Executor subclass for running simulation tasks locally using Dask.

    Attributes:
        client (dask.distributed.Client): The Dask client used for task submission and execution.
        simulator_client (dask.distributed.Client): Client used specifically for simulation tasks.
        surrogate_client (dask.distributed.Client): Client used for training surrogate models.
        clients (list): A list of Dask clients.
        max_samples (int): The maximum number of samples to be generated.
        base_run_dir (str): The base directory where simulation results will be stored.
        sampler (object): The sampler object used to generate parameter sets for simulation runs.
        runner_args (dict): Arguments required to initialize the runner.
    """

    def __init__(self, **kwargs):
        """
        Initializes the LocalDaskExecutor.
    ...
    def submit_batch_of_params(self, param_list: list[dict]) -> list:
        """
        Submits a batch of simulation tasks to the Dask client.

        Args:
            param_list (list[dict]): A list of parameter sets for simulation runs.

        Returns:
            list: A list of futures representing the submitted simulation tasks.
        """
        futures = []
    ...
    def start_runs(self):
        """
        Initiates the simulation runs based on the sampling interface specified.
        """
        ...
        if sampler_interface in [S.SEQUENTIAL]:
            ...
            while True:
                ...
                outputs = self.sampler.collect_batch_results(
                    outputs
                )  # TODO: this should probably be in parser
                ...
                print(
                    "Updated Pool size: ",
                    len(self.sampler.parser.pool),
                    "Train size: ",
                    len(self.sampler.parser.train),
                )
                ...
                example_model = create_model(
                    self.sampler.model_kwargs
                )  # nn.Sequential(nn.Linear(4, 100), nn.ReLU(), nn.Linear(100, 100), nn.ReLU(), nn.Linear(100, 1))
