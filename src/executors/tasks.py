...
def run_simulation_task(runner_args:dict, run_dir:str, params: dict=None, index=None, future=None) -> dict:
        """
        ...
        try:            
            ...
            # and we can simply run the code. This will happen if the executor calling this function is second (or third etc) in a pipeline
                runner_output = runner.single_code_run(run_dir, index=index)
            else:
                if run_dir != None:
                    if not os.path.exists(run_dir):
                        os.mkdir(run_dir)        
                runner_output = runner.single_code_run(run_dir=run_dir, params=params, index=index) 
